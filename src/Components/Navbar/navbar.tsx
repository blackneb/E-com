  ...
  return (
    ...
    <nav className='shadow-md w-full sticky top-0 left-0'>
        <div className='md:flex items-center justify-between bg-white py-4 md:px-10 px-7'>
            ...
            <ul className={`md:flex md:items-center md:pb-0 pb-12 absolute md:static md:bg-white bg-purple-600 md:z-auto z-[-1] left-0 w-full md:w-auto md:pl-0 transition-all duration-500 ease-in pt-10 ${open ? 'top-15 ':'top-[-490px]'}`}>
                {
                    links.map((link) => (
                        <li key={link.name} className='md:ml-8 text-sm md:my-0 my-1 md:text-purple-600 text-white mt-3' >
                            {link.name}
                        </li>
                    ))
                }
